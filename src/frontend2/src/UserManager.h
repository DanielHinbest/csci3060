...
public:
    ...
    User createUser()
    {
        ...
        // AccountsManager tries to create admin type user
        if (currentUser.getType() == AccountsManager && userType == 1) {
            std::cout << "Cannot create admin users." << std::endl;
            return User("", 0, 0.0);
        }
        ...
        return newUser; // why not just zero index :( bug festival
    }

    // Function to delete a user
    User deleteUser()
    {
        // Check if the current user is an admin or AccountsManager
        if (currentUser.getType() != Admin && currentUser.getType() != AccountsManager)
        {
        ...
        // If AccountsManager user tries to delete admin user
        if (userToDelete->getType() == Admin && currentUser.getType() == AccountsManager) {
            std::cout << "Cannot delete Admin users." << std::endl;
            return User("", 0, 0.0);
        }
    ...
    void listUsers()
    {
        ...
        std::cout << std::setw(20) << std::left << "Username"
                  << std::setw(20) << std::left << "User Type"
        ...
        for (const User &user : users)
        {
            ...
            std::cout << std::setw(20) << std::left << user.getUsername()
                      << std::setw(20) << std::left << getFullUserType(user.getType())
    ...
    std::vector<User> searchUsers(const std::string& searchString) 
    {
        std::vector<User> foundUsers;
        for (const User& user : users) {
            if (user.getUsername().find(searchString) != std::string::npos) 
            {
...
private:
    ...
    // BIG BUG IN GIVEN FRONTEND, would create wrong user type
    int getType()
    {
        int userType;
        while (true)
        {
            std::cout << "Enter user type (1-admin, 2-full-standard, 3-buy-standard, 4-sell-standard, 5-accounts-manager): ";
            std::cin >> userType;
