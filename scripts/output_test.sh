...
for input_file in *.inp; do
    ...
    if [ $iteration_count -eq 47 ]; then # For the 47th iteration (the empty list of games test), use an empty gamescollection file
        ../../../../distribution-system.exe ../test_data/currentaccounts_test.txt ../test_data/availablegames_test_empty.txt ../test_data/gamescollection_test.txt ../test_data/dailytransactions_test.txt < <(cat "$input_file"; echo "exit") > temp_output.txt 2>&1
    else
        ../../../../distribution-system.exe ../test_data/currentaccounts_test.txt ../test_data/availablegames_test.txt ../test_data/gamescollection_test.txt ../test_data/dailytransactions_test.txt < <(cat "$input_file"; echo "exit") > temp_output.txt 2>&1
    fi
